...
const PhaserGame = () => {
  ...
  useEffect(() => {
    ...
    class LetterScene extends Phaser.Scene {
      ...
      //   });
      // }
// showStarCollectAnimation(letter) {
//   const centerX = this.cameras.main.centerX;
//   const centerY = this.cameras.main.centerY;
...
//   // Reward image bouncing animation
//   const rewardImage = this.add.image(centerX, centerY, rewardKey).setScale(0.5).setDepth(1);
...
//   // Glowing reward text
//   const name = this.rewardNameMap[letter] || letter;
//   const rewardText = this.add.text(centerX, centerY + 120, `✨ You got a ${name}! ✨`, {
//     fontSize: '28px',
//     color: '#ffff00',
//     fontStyle: 'bold',
//     stroke: '#ffcc00',
//     strokeThickness: 4,
//     shadow: { fill: true, color: '#ffaa00', blur: 10, offsetX: 0, offsetY: 0 },
//   })
//   .setOrigin(0.5)
//   .setDepth(2);
...
//   // Sparkling stars animation around the reward image
//   const stars = [];
//   for (let i = 0; i < 10; i++) {
//     const star = this.add.star(
//       centerX + Phaser.Math.Between(-100, 100),
//       centerY + Phaser.Math.Between(-80, 80),
//       5,
//       5,
//       10,
//       0xffff00,
...
//       onRepeat: () => {
//         star.x = centerX + Phaser.Math.Between(-100, 100);
//         star.y = centerY + Phaser.Math.Between(-80, 80);
//         star.scale = 1;
//         star.alpha = 1;
//       },
//     });
...
//   // Cleanup and move to next letter after 3 seconds
//   this.time.delayedCall(3000, () => {
//     rewardImage.destroy();
//     rewardText.destroy();
...
showStarCollectAnimation(letter) {
  ...
  // Lower BGM volume smoothly before playing reward sound
  this.tweens.add({
    ...
    ease: "Power1",
    onComplete: () => {
      // Play reward sound at reduced volume
      this.sound.play(rewardKey, { volume: 0.5 });

      // Restore BGM volume after 4 seconds
  ...
  // Reward image with initial alpha 0 (invisible)
  const rewardImage = this.add
    .image(centerX, centerY, rewardKey)
    .setScale(0.5)
    .setDepth(1)
  ...
  // Reward text with initial alpha 0
  const name = this.rewardNameMap[letter] || letter;
  const rewardText = this.add
    .text(centerX, centerY + 120, `✨ You got a ${name}! ✨`, {
      fontSize: "28px",
  ...
  // Fade in reward text with glow effect (pulsating alpha)
  this.tweens.add({
    targets: rewardText,
    alpha: 1,
    duration: 800,
  ...
  this.time.delayedCall(4200, () => {
    this.tweens.add({
      targets: [rewardImage, rewardText],
      alpha: 0,
      duration: 800,
      ease: "Power1",
      onComplete: () => {
          ...
          if ((isPlayerA && isLetterB) || (isPlayerB && isLetterA)) {
            ...
            if (letterSprite) {
              ...
              if (this.clickCount >= this.maxRepeats && !this.isRewarding) {
                ...
                this.showStarCollectAnimation(currentLetter);
              }
            }
          }
        });
